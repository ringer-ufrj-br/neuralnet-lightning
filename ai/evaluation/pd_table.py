...
def to_latex(
    ...
    Args:
        ...
        label (Optional[str]): LaTeX label. Defaults to 'tab:pd_table_<operating_point>'.
    ...
    return _latex_from_wide(
        wide, caption, label or f"tab:pd_table_{operating_point}",
...
def integrated_to_latex(
    ...
    model_names: Optional[Sequence[str]] = None,
    caption: Optional[str] = None,
    label: str = "tab:pd_table_integrated",
    decimals: int = 2,
    highlight_pd: bool = True,
    operating_points: Optional[Sequence[str]] = None
) -> str:
    ...
    Args:
        ...
        model_names (Optional[Sequence[str]]): Models to show, in row order.
        caption (Optional[str]): Table caption. Generated when None.
        label (str): LaTeX label. Defaults to 'tab:pd_table_integrated'.
        decimals (int): Decimal places. Defaults to 2.
...
def default_output_dir(results_root: str, model_names: Optional[Sequence[str]]) -> str:
    ...
    if model_names and len(model_names) == 1:
        return os.path.join(results_root, model_names[0], "pd_table")
    return os.path.join(results_root, "comparison", "pd_table")
...
def build_report(
    ...
    Args:
        ...
        output_dir (str): Where the table artefacts go. Defaults to the model's own
            'pd_table' directory, or '<results_root>/comparison/pd_table' for several models.
    ...
    for point in points:
        if "tex" in formats:
            ...
            if source:
                tex_path = os.path.join(output_dir, f"pd_table_{point}.tex")
        ...
        for extension in (fmt for fmt in formats if fmt != "tex"):
            rendered = render_figure(
                agg, point, os.path.join(output_dir, f"pd_table_{point}.{extension}"),
    ...
    if integrated:
        ...
        if not integrated_long.empty:
            ...
            if "tex" in formats:
                ...
                if source:
                    tex_path = os.path.join(output_dir, "pd_table_integrated.tex")
                    with open(tex_path, "w") as handle:
                        handle.write(source)
            ...
            for extension in (fmt for fmt in formats if fmt != "tex"):
                rendered = render_integrated_figure(
                    agg_integrated,
                    os.path.join(output_dir, f"pd_table_integrated.{extension}"),
